...
/* int main(int argc, char *argv[]){ */ // not necessary!
#ifdef __cplusplus
extern "C"
#endif
int main(){
    ...
    Try_the_basics_with_SDL_Init(Nope); // just try to init, no windows open
    Try_window_management(Yep); // open windows for manual tests
    return UNITY_END();
}
